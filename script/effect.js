...
function MoneyFactory() {
	...
	this.getMoney = function(monster) {
		var money_data = window.monsters_attr[monster.name].things["Ouro"];
...
function ThingsFactory() {
	...
	this.getRes = function(name) {
		switch(name) {
			...
			case "Navalha":
				return window.resource.things["navalha"];
...
function TipsFactory() {
	...
	this.createExpTip = function(exp) {
		return new Tip("ganho de experiência(+" + exp + ")", 6);
	}

	this.createThingTip = function(tip_data) {
		switch(tip_data.type) {
			case 0:
				return new Tip("obter moedas de ouro(+" + tip_data.money + ")", 0);
			case 1:
				return new Tip("obter equipamento(" + tip_data.name +")", 1);
			case 2:
				var tip = "";
				if (tip_data.amount > 1) {
					tip = "pegar" + tip_data.name + "(+" + tip_data.amount + "个)";
				} else {
					tip = "pegar" + tip_data.name;
				}
	...
	this.createFullTip = function() {
		return new Tip("Não foi possível pegar os itens, a mochila está cheia", 5);
	}

	this.createSkillTip = function() {
		return new Tip("MP insuficiente, incapaz de usar habilidades", 5);
	}
